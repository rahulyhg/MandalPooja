...
public class StateSpinnerList {
    ...
    int count;
    public StateSpinnerList(YaatraList yaatraList) {
        context = yaatraList;
    }

    public StateSpinnerList(Register register) {
        context = register;
    }

    public void FetchAllState(List<String> stateNameList, List<String> stateIdList, CountrySpinnerAdapter countrySpinnerAdapter, String nameOfCountry, ProgressDialog stateDialogBox, int counter) {
        allNameList = stateNameList;
        allIdList =stateIdList;
        SpinnerAdapterAsynch = countrySpinnerAdapter;
        countryName = nameOfCountry;
        progressDialogBox = stateDialogBox;

        count = counter;

        if(count == 1){
            webMethodName = "listState";
        }else{
            webMethodName = "listStateMaster";
        }
    ...
    public static class AsyncCallWS extends AsyncTask<String, Void, Void> {
        ...
        protected Void doInBackground(String... params) {
            ResponseResult = WebService.AllState(countryName,webMethodName);
            return null;
        }
        ...
        protected void onPostExecute(Void res) {
            ...
            if (ResponseResult.isEmpty()) {
                ...
                builder.setMessage("Unable to Fetch State.");
                ...
                try {
                    ...
                    //allIdList.clear();
                    allNameList.add("Select State");
                    allIdList.add("0");
                    for (int i = 0; i < jsonArray.length(); i++) {
                        try {
                            JSONObject obj = jsonArray.getJSONObject(i);
                            allNameList.add(obj.getString("stateName"));
                            allIdList.add(obj.getString("stateid"));
